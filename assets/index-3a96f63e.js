import"./common-173efd62.js";import{i as N}from"./business-wasm-4c3a1904.js";const h={WIN_AMOUNT_PER_NUMBER:45,buyerId:0,betItemId:0,currentView:StorageManager.get("currentView","ascending"),calculateWinningsFromRecords(e,t=!1){if(e.length===0)return{totalBet:0,numberCounts:{},numberBetAmounts:{},numberWinnings:{},topThree:[],topThreeDetails:[]};const i=t?e.filter(l=>!l.fromPrediction):e;if(i.length===0)return{totalBet:0,numberCounts:{},numberBetAmounts:{},numberWinnings:{},topThree:[],topThreeDetails:[]};const n={},o={};let r=0;i.forEach(l=>{r+=l.total,l.numbers.forEach(d=>{n[d]||(n[d]=0,o[d]=0),n[d]++,o[d]+=l.amount})});const s={};Object.keys(o).forEach(l=>{s[l]=o[l]*this.WIN_AMOUNT_PER_NUMBER});const a=Object.entries(s).sort((l,d)=>d[1]-l[1]).slice(0,3),c=a.map(([l,d])=>{const m=o[l],u=n[l],p=d-r;return{number:l,betCount:u,betAmount:m,winningAmount:d,totalBet:r,profit:p}});return{totalBet:r,numberCounts:n,numberBetAmounts:o,numberWinnings:s,topThree:a,topThreeDetails:c}}},B={renderNumberSelector(e,t,i){return h.currentView==="zodiac"?this.renderZodiacView(e,t,i):this.renderAscendingView(e,t)},renderAscendingView(e,t){return t.map(i=>`<div class="number-item ${i.color}" data-value="${i.value}" onclick="BettingApp.toggleNumber(${e}, '${i.value}', 'manual')">${i.value} ${i.zodiacName}</div>`).join("")},renderZodiacView(e,t,i){return DataReader.getNumbersByZodiac(i).map(o=>`
    ...
    `},renderBuyerSummary(e,t,i,n=null,o=!1){if(i.topThreeDetails.length===0)return`
      ...
      <table class="table-margin-bottom">
        ...
        <tbody>
    `;return r.forEach(a=>{const c=a.id,l=a.fromPrediction||!1;let d="";if(l){const m=a.predictionTier||"",u=m==="1"?"一":m==="2"?"二":m==="3"?"三":"",p=u?`(${u})`:"";d=`<span class="prediction-badge" title="来源于${u||"推断"}推断">推断${p}</span>`}s+=`
        <tr>
          <td>#${c}${d}</td>
...
`),r=t?t.target:document.querySelector(`button[onclick*="copyBettingData('${e}')"]`);(a=>{if(navigator.clipboard&&navigator.clipboard.writeText)return navigator.clipboard.writeText(a);{const c=document.createElement("textarea");c.value=a,c.style.position="fixed",c.style.opacity="0",c.style.left="-9999px",document.body.appendChild(c),c.select();try{return document.execCommand("copy"),document.body.removeChild(c),Promise.resolve()}catch(l){return document.body.removeChild(c),Promise.reject(l)}}})(o).then(()=>{if(r){const a=r.textContent;r.textContent="已复制",r.style.background="#27ae60",r.style.color="white",r.style.borderColor="#27ae60",setTimeout(()=>{r.textContent=a,r.style.background="white",r.style.color="#666",r.style.borderColor="#ddd"},2e3)}}).catch(a=>{console.error("复制失败:",a),alert("复制失败，请手动复制")})},buildBettingUniqueId(e,t,i,n,o){const r=(o||"").replace(/[\s|]/g,"_").slice(0,50);return[e,t,i,n,r].join("_").toLocaleLowerCase()},buildSubmitDataFromRecords(e,t,i,n){const o={};return n.forEach(r=>{const s=`${r.buyerId}_${r.buyerName}`;o[s]||(o[s]={buyerId:r.buyerId,buyerName:r.buyerName,betItems:[]}),o[s].betItems.push({numbers:r.numbers,amount:r.amount,total:r.total,fromPrediction:r.fromPrediction,predictionTier:r.predictionTier})}),{basicInfo:{region:e,regionName:B.getRegionName(e),year:t,period:i},buyers:Object.values(o).map(r=>{const s=n.filter(c=>c.buyerId===r.buyerId),a=h.calculateWinningsFromRecords(s);return{buyerId:r.buyerId,buyerName:r.buyerName,betItems:r.betItems,summary:{totalBet:a.totalBet,topThreeDetails:a.topThreeDetails}}})}},saveBettingToLocalStorage(e,t){e.buyers.forEach(i=>{if(i.buyerName){const n={basicInfo:e.basicInfo,buyer:i};StorageManager.saveBettingData(i.buyerName,t,n),console.log(`选号数据已保存: ${i.buyerName} - ${t}`)}})},getPredictionForPayload(e,t){const i=StorageManager.getPredictionData(e,t);return!i||!i.predictionData&&!i.results?null:{predictionData:i.predictionData,results:i.results,materialId:i.materialId||null,materialLabels:this.MATERIAL_LABELS,forecastSource:i.forecastSource||null}},buildPayloadForBuyer(e,t,i,n,o,r){const s=this.buildBettingUniqueId(n,o,r,i,e.buyerName),a=this.getPredictionForPayload(e.buyerName,i);return{uniqueId:s,buyerName:e.buyerName,date:i,basicInfo:t.basicInfo,betting:{buyerId:e.buyerId,betItems:e.betItems,summary:e.summary},prediction:a}},submitBetting(e,t,i,n,o,r){const s=typeof window<"u"?window.SupabaseDb:null;if(!s||!s.getClient())return alert("Supabase 未配置，无法提交。请在 supabase-config.js 中填写 SUPABASE_URL 与 SUPABASE_ANON_KEY。"),Promise.resolve();const a=t.buyers.filter(c=>c.buyerName).map(c=>{const l=this.buildPayloadForBuyer(c,t,r,i,n,o),d=JSON.stringify(l);return s.insertSubmission(e,{unique_id:l.uniqueId,buyer_name:l.buyerName,date:l.date,payload:d})});return Promise.all(a).then(c=>{const l=c.filter(d=>d&&d.error);l.length===0?(alert("选号已提交"),document.querySelectorAll(".buyer-name-input").forEach(d=>{d.value=""}),this.updateTable()):(console.error("Supabase 提交部分失败",l),alert("部分提交失败，请查看控制台。本地已保存。"))}).catch(c=>{console.error("Supabase 提交失败",c),alert("提交到服务器失败："+(c.message||"网络错误")+"。本地已保存。")})},submitData(){const e=typeof Auth<"u"?Auth.getFormToken():null;if(!e){alert("请先到登录页填写 FORM TOKEN 后再提交选号。");return}const t=document.getElementById("region"),i=document.getElementById("year"),n=document.getElementById("period");if(!t||!i||!n)return;const o=t.value,r=parseInt(i.value,10),s=(n.value||"").trim(),a=this.collectBetRecords();if(!a||a.length===0){alert("暂无选号数据，请先选择猿份号并填写分量。");return}const c=this.buildSubmitDataFromRecords(o,r,s,a);if(!confirm("确定要提交选号数据吗？提交后将保存到服务器。"))return;console.log("提交的选号数据：",JSON.stringify(c,null,2)),console.log("提交的选号数据（对象）：",c);const l=StorageManager.getCurrentDate();this.saveBettingToLocalStorage(c,l),this.submitBetting(e,c,o,r,s,l)},handleAmountInput(e){const t=document.getElementById(`bet-amount-${e}`);if(!t)return;const i=t.value;i&&!/^\d+$/.test(i)&&(t.value=i.replace(/\D/g,""));const n=parseInt(t.value,10);!isNaN(n)&&n>99999&&(t.value="99999")},validateAmount(e){const t=document.getElementById(`bet-amount-${e}`),i=document.getElementById(`error-amount-${e}`);if(!t||!i)return;const n=t.value.trim();if(t.classList.remove("error"),i.classList.remove("show"),n==="")return;const o=parseInt(n,10);return isNaN(o)||o<=0||o>=1e5||o!==parseFloat(n)?(t.classList.add("error"),i.classList.add("show"),!1):(this.updateTable(),!0)},addBetItem(e){h.betItemId++;const t=h.betItemId,i=parseInt(document.getElementById("year").value),n=DataReader.getAllNumbers(i),o=document.createElement("div");o.className="bet-item",o.id=`bet-item-${t}`,o.dataset.buyerId=e,o.innerHTML=B.renderBetItem(t,e,n,i),document.getElementById(`bet-items-${e}`).prepend(o),o.classList.add("bet-item-new"),setTimeout(()=>{o.classList.remove("bet-item-new")},2e3),this.updateEmptyState(e)},removeBetItem(e){const t=document.getElementById(`bet-item-${e}`);if(t){const i=t.dataset.buyerId,n=t.querySelectorAll(".number-item.selected").length>0,o=document.getElementById(`bet-amount-${e}`),r=o&&parseFloat(o.value)||0;if((n||r>0)&&!window.confirm("检测到该选号项包含已选择的猿份号或已填写分量，确定要删除吗？"))return;t.remove(),this.updateEmptyState(i),this.updateTable()}},updateEmptyState(e){const t=document.getElementById(`bet-items-${e}`);if(!t)return;const i=t.querySelectorAll(".bet-item").length>0,n=`bet-items-empty-${e}`;let o=document.getElementById(n);i?o&&o.remove():o||(o=document.createElement("div"),o.id=n,o.className="bet-items-empty",o.textContent="暂无选号申请，点击“添加选号”开始。",t.appendChild(o))},toggleNumber(e,t){const i=document.querySelector(`#number-selector-${e} .number-item[data-value="${t}"]`),n=i.classList.contains("selected");i.classList.toggle("selected"),arguments[2]==="manual"&&(n?delete i.dataset.manual:i.dataset.manual="1"),this.updateSelectedNumbers(e),this.updateTable()},updateSelectedNumbers(e){const t=document.querySelectorAll(`#number-selector-${e} .number-item.selected`),i=Array.from(t).map(o=>o.dataset.value),n=document.getElementById(`selected-numbers-${e}`);i.length===0?n.innerHTML='<span class="empty-state-text">未选择任何数字</span>':n.innerHTML=i.map(o=>`<span class="selected-number-tag">${o}</span>`).join("")},updateAllNumberSelectors(){const e=parseInt(document.getElementById("year").value),t=DataReader.getAllNumbers(e);document.querySelectorAll(".bet-item").forEach(n=>{const o=n.id.split("-")[2],r=document.getElementById(`number-selector-${o}`);if(r){const s=Array.from(r.querySelectorAll(".number-item.selected")).map(a=>a.dataset.value);h.currentView==="zodiac"?r.classList.add("zodiac-view"):r.classList.remove("zodiac-view"),r.innerHTML=B.renderNumberSelector(o,t,e),s.forEach(a=>{const c=r.querySelector(`.number-item[data-value="${a}"]`);c&&(c.classList.add("selected"),c.dataset.manual="1")}),this.updateSelectedNumbers(o)}})},updateAllPredictionNumberSelectors(){parseInt(document.getElementById("year").value),document.querySelectorAll(".buyer-card").forEach(t=>{const i=t.id.split("-")[1],n=document.getElementById(`pred-special-selector-${i}`);if(n){const o=Array.from(n.querySelectorAll(".number-item.selected")).map(r=>r.dataset.value);n.innerHTML=B.renderPredictionNumberSelector(i),o.forEach(r=>{const s=n.querySelector(`.number-item[data-value="${r}"]`);s&&s.classList.add("selected")})}})},collectBetRecords(){const e=[],t=document.getElementById("region"),i=document.getElementById("year"),n=document.getElementById("period");if(!t||!i||!n)return e;const o=t.value,r=parseInt(i.value,10),s=(n.value||"").trim();return document.querySelectorAll(".buyer-card").forEach(c=>{const d=(c.id||"").split("-")[1];if(d==null)return;const m=document.getElementById(`buyer-name-${d}`),u=m&&m.value?String(m.value).trim():"";if(!u)return;c.querySelectorAll(".bet-item").forEach(g=>{const f=(g.id||"").split("-")[2];if(f==null)return;const b=document.getElementById(`bet-amount-${f}`);if(!b)return;const S=g.querySelectorAll(".number-item.selected"),y=Array.from(S).map(E=>E.dataset.value).filter(Boolean),$=parseFloat(b.value,10)||0;if(y.length>0&&$>0){const E=g.dataset.fromPrediction==="true",A=g.dataset.predictionTier||null;e.push({id:f,buyerId:d,buyerName:u,region:o,year:r,fromPrediction:E,predictionTier:A,period:s,numbers:y,amount:$,total:y.length*$})}})}),e},updateSubmitButtonState(){const e=document.querySelector(".submit-btn");if(!e)return;const t=this.collectBetRecords();e.disabled=t.length===0},updateTable(){const e=this.collectBetRecords(),t={};e.forEach(n=>{const o=`${n.buyerId}_${n.buyerName}`;t[o]||(t[o]={buyerId:n.buyerId,buyerName:n.buyerName,region:n.region,year:n.year,period:n.period,records:[]}),t[o].records.push(n)}),document.querySelectorAll(".buyer-card").forEach(n=>{const o=n.id.split("-")[1],r=document.getElementById(`buyer-summary-${o}`);if(!r)return;const s=document.getElementById(`summary-top-three-${o}`);document.getElementById(`summary-toggle-${o}`);const a=s&&s.classList.contains("collapsed"),c=Object.keys(t).find(u=>{const p=t[u];return p&&p.buyerId===o});if(!c||t[c].records.length===0){r.classList.add("buyer-summary-hidden");return}const l=t[c],d=l.records,m=h.calculateWinningsFromRecords(d,!1);if(r.innerHTML=B.renderBuyerSummary(l.buyerName,l,m,d,!1),r.classList.remove("buyer-summary-hidden"),a){const u=document.getElementById(`summary-top-three-${o}`),p=document.getElementById(`summary-toggle-${o}`);u&&p&&(u.classList.add("collapsed"),p.classList.add("collapsed"))}}),this.updateTotalSummary(t),this.updateSubmitButtonState()},updateTotalSummary(e){const t=document.getElementById("totalSummary"),i=document.getElementById("totalSummaryContent");if(!t||!i)return;const n=Object.values(e);if(n.length===0){t.classList.remove("show");return}this.currentDimension==="buyer"?this.renderBuyerDimension(n,i):this.renderNumberDimension(n,i),t.classList.add("show")},renderBuyerDimension(e,t){const i=[];let n=0;e.forEach(r=>{const s=r.records,a=r.buyerName||"未命名";let c=0;s.forEach(m=>{c+=m.total});const l={};s.forEach(m=>{m.numbers.forEach(u=>{l[u]||(l[u]={number:u,amount:m.amount,betCount:0,numberBetAmount:0,winningAmount:0,profit:0}),l[u].betCount+=1,l[u].numberBetAmount+=m.amount})});const d=Object.values(l).map(m=>(m.winningAmount=m.numberBetAmount*h.WIN_AMOUNT_PER_NUMBER,m.profit=m.winningAmount-c,m));d.sort((m,u)=>{const p=parseInt(m.number),g=parseInt(u.number);return p-g}),i.push({buyerName:a,buyerTotalCost:c,numbers:d}),n+=c}),i.sort((r,s)=>r.buyerName.localeCompare(s.buyerName));let o=`<div class="summary-top-three" id="total-summary-table">
    ...
    `,t.innerHTML=o},renderNumberDimension(e,t){const i={};let n=0;e.forEach(s=>{s.records.forEach(c=>{c.numbers.forEach(l=>{i[l]||(i[l]={number:l,totalBetAmount:0,betCount:0,winningAmount:0}),i[l].totalBetAmount+=c.amount,i[l].betCount+=1,n+=c.amount})})}),Object.values(i).forEach(s=>{s.winningAmount=s.totalBetAmount*h.WIN_AMOUNT_PER_NUMBER});let o=Object.values(i);this.currentSort==="number"?o.sort((s,a)=>{const c=parseInt(s.number),l=parseInt(a.number);return c-l}):o.sort((s,a)=>a.winningAmount-s.winningAmount);let r=`<div class="summary-top-three" id="total-summary-table">
